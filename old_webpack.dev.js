...
module.exports = () => merge(commonConf({ outputFile, assetFile }), {
    ...
    plugins: [
        ...
        new HtmlWebpackPlugin({
            template: path.join(__dirname, './src/html/43_dom_test.html'),
            filename: '43_dom_test.html',
            inject: 'body', //
            chunks: ['app'], //←ここでscssファイルを指定してあげる
        }),
        new HtmlWebpackPlugin({
            template: path.join(__dirname, './src/html/44_class_object.html'),
            filename: '44_class_object.html',
            inject: 'body', //
            chunks: ['app'], //←ここでscssファイルを指定してあげる
        }),
        new HtmlWebpackPlugin({
            template: path.join(__dirname, './src/html/45_tween_animation.html'),
            filename: '45_tween_animation.html',
            inject: 'body', //
            chunks: ['app'], //←ここでscssファイルを指定してあげる
        }),
        new HtmlWebpackPlugin({
            template: path.join(__dirname, './src/html/50_intersectionObserver.html'),
            filename: '50_intersectionObserver.html',
            inject: 'body', //
            chunks: ['app'], //←ここでscssファイルを指定してあげる
        }),
        new HtmlWebpackPlugin({
            template: path.join(__dirname, './src/html/51_scroll_animation.html'),
            filename: '51_scroll_animation.html',
            inject: 'body', //
            chunks: ['app'], //←ここでscssファイルを指定してあげる
        }),
    ],
